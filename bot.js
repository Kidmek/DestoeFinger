...
const createDelPlayers = async (message, key) => {
  try {
    ...
    } else if (message === 'delete') map.delete(key)
  } catch (error) {
...
const getKey = (msg) => {
  var key
  if (msg.chat) return (key = msg.chat.id)
  ...
  try {
    if (msg.text) {
      if (msg.text.indexOf('/menu') === 0 || msg.text.indexOf('/start') === 0) {
        ...
        if (map.get(getKey(msg))) createDelPlayers('delete', msg)
        ...
        msg.text.indexOf(desto) === 0 &&
        !msg.text.includes('Desto') &&
        !msg.text.includes(finger)
      ) {
        compareHands(msg, 1)
      } else if (
        msg.text.indexOf(finger) === 0 &&
        !msg.text.includes('Finger') &&
        !msg.text.includes(couter)
      ) {
        compareHands(msg, 2)
      } else if (msg.text.indexOf(cow) === 0 && !msg.text.includes('Cow')) {
        compareHands(msg, 3)
      } else if (
        msg.text.indexOf(couter) === 0 &&
        !msg.text.includes('Cowter')
      ) {
        compareHands(msg, 4)
        ...
        if (msg.chat.type.includes('private'))
          bot.sendMessage(
            msg.chat.id,
            'Please press /start or /menu to go to the Main Menu.'
          )
      }
    }
  } catch (error) {
  ...
  try {
    if (msg.data.indexOf('Practice') === 0) {
      ...
      if (msg.from.username === 'KidusM') {
        bot.editMessageText('Good Luck ', {
          inline_message_id: msg.inline_message_id,
        })
        tournament(msg, 'new')
      } else
        return bot.answerCallbackQuery(msg.id, {
          text: 'Only The Admin Can Start The Tournament',
          show_alert: false,
        })
    } else if (
      msg.data.includes('2p') ||
      msg.data.includes('3p') ||
      msg.data.includes('4p') ||
      msg.data.includes('5p')
    ) {
      var key = getKey(msg)
      ...
      var playerName = msg.from.username
        ? '@' + msg.from.username
        : msg.from.last_name
...
bot.on('inline_query', (msg) => {
  try {
    if (msg.query.includes('start') || msg.query.includes('Start')) {
      ...
      msg.query.includes('tournament') &&
      msg.from.username === 'KidusM'
    ) {
      createDelPlayers('players')
      bot.answerInlineQuery(msg.id, [
        {
          ...
          input_message_content: {
            message_text: 'Click Join to participate in the Tournament',
          },
          reply_markup: {
            inline_keyboard: [
              [{ text: 'Join', callback_data: 'joinTournament' }],
      ...
      if (mass) {
        ...
        if (
          !mass.includes(query[1]) ||
          !tournamentStatus.tournamentPlayers.includes(query[1])
        ) {
        ...
        let i = 0,
          names = ''
        while (i < 5) {
          if (mass[i] !== query[1] && mass[i]) names += mass[i] + ' , '
          i++
        }
        ...
        if (
          tournamentStatus.amountOfGames === 0 &&
          tournamentStatus.tournamentPlayers.length > 1
        ) {
      ...
      if (query.pop() === 'begin') {
        query.forEach((p) => {
          if (p !== 'new') tournamentPlayers.push(p)
        })
        map.set('tournament', JSON.stringify(tournamentStatus))
        tournament(msg, 'editted')
      }
...
const practice = async (msg) => {
  try {
    bot
      .sendMessage(msg.message.chat.id, 'First Pick ', {
        reply_to_message_id: msg.message_id,
        reply_markup: {
          keyboard: [
            [desto + 'Desto', finger + 'Finger', cow + 'Cow'],
  ...
  try {
    ...
    if (newPlayers.players[0].pick[0] === h) {
      ...
      var playerName = msg.from.username
        ...
        ? `${msg.from.first_name}` + `${msg.from.last_name}`
        : `${msg.from.first_name}`
      await Games.findByIdAndUpdate(newPlayers.id, {
        $push: { players: playerName },
      })
      newPlayers.players[0].pick[0] = h
      newPlayers.players[0].name = msg.chat.first_name
      map.set(getKey(msg), JSON.stringify(newPlayers))
      return bot.sendMessage(msg.chat.id, 'Pick a Second One ', {
        reply_to_message_id: msg.message_id,
      })
    } else if (
      !newPlayers.players[0].pick[1] &&
      newPlayers.players[0].pick[0] !== h
    ) {
...
const startPlayingWithBot = async (msg) => {
  try {
    ...
    await Games.findByIdAndUpdate(newPlayers.id, { $push: { players: 'Bot' } })
    ...
    bot.sendMessage(
      msg.chat.id,
      'The Bot has Picked : ' +
        hands[newPlayers.players[1].pick[0] - 1] +
        ' and ' +
        hands[newPlayers.players[1].pick[1] - 1] +
        '\nYou have Picked : ' +
        hands[newPlayers.players[0].pick[0] - 1] +
        ' and ' +
        hands[newPlayers.players[0].pick[1] - 1] +
        '\nStart Playing ',
      {
        reply_to_message_id: msg.message_id,
        reply_markup: {
...
const compareHands = async (msg, h) => {
  try {
    ...
    if (newPlayers) {
      if (
        newPlayers.players[0].score > 10 ||
        newPlayers.players[1].score >= ENDPOINT ||
        newPlayers.players[0].score >= ENDPOINT
      ) {
        map.set(getKey(msg), JSON.stringify(newPlayers))
        return bot.sendMessage(
          msg.chat.id,
      ...
      if (newPlayers.players[0].pick.includes(result)) {
        ...
        await bot.sendMessage(
          ...
          hands[result - 1] +
            ...
            newPlayers.players[0].name +
            ' ' +
            newPlayers.players[0].score +
            ' - ' +
            newPlayers.players[1].score +
            ' ' +
        ...
        !newPlayers.players[0].pick.includes(result) &&
        !newPlayers.players[1].pick.includes(result)
      )
        bot
          .sendMessage(msg.chat.id, handSigns[botHand - 1] + '\n', {
            reply_to_message_id: msg.message_id,
          })
          .then(() => {
            bot.sendMessage(
    ...
    } else bot.sendMessage(msg.chat.id, 'Send /start to start playing again.')
  } catch (error) {
...
const multiKeyboard = (mode, h) => {
  if (mode === 'multiPlayer') {
    ...
    return JSON.stringify({
      inline_keyboard: [[{ text: 'Play Again', callback_data: 'multiAgain' }]],
    })
  } else if (mode === 'inline') {
    return JSON.stringify({
      inline_keyboard: [
  ...
  try {
    ...
    if (msg.chat_instance === INSTANCE) {
      ...
      if (query.includes('Final')) {
        query[1] = 1
        game = 'Final Game '
      } else game = 'Game ' + parseInt(query[1]) + ' '
      round = JSON.parse(map.get('tournament')).round
      map.get(parseInt(query[1])).forEach((p) => {
        if (typeof p !== 'object' && i < 5) players[i] = p
        i++
      })
      tournamentPlayers = JSON.parse(map.get('tournament')).tournamentPlayers
      if (!map.has(key)) createDelPlayers('create', key)
      newPlayers = JSON.parse(map.get(key))
      newPlayers.players.forEach((p) => names.push(p.name + ' , '))
      if (!tournamentPlayers.includes(playerName))
        return bot.answerCallbackQuery(msg.id, {
          text: 'Wait for the next Tournament',
          show_alert: false,
        })
      else if (
        tournamentPlayers.includes(playerName) &&
        names.includes(`${playerName} , ` && newPlayers.totalPlayers > 2)
      )
        return bot.answerCallbackQuery(msg.id, {
          text: 'Already Picked Yours',
          show_alert: false,
        })
      else if (
        tournamentPlayers.includes(playerName) &&
        ...
        return bot.answerCallbackQuery(msg.id, {
          text: 'Look For Your Own Game',
          show_alert: false,
        })
      if (!newPlayers.totalPlayers) newPlayers.totalPlayers = players.length
      map.set(key, JSON.stringify(newPlayers))
    }
    ...
    if (
      (id.includes(msg.from.id) && newPlayers.players[n].pick[1]) ||
      (id.includes(msg.from.id) && newPlayers.totalPlayers > 2)
    ) {
    ...
    if (
      newPlayers.players[p - 1].pick[0] &&
      newPlayers.players[p - 1].id !== msg.from.id
    ) {
      bot.answerCallbackQuery(msg.id, {
        text: 'Wait For Your Turn To Pick',
        show_alert: false,
      })
      return
    } else if (newPlayers.players[p - 1].pick[0] === h) {
      bot.answerCallbackQuery(msg.id, {
        text: 'You Cant Pick The Same Hand Twice , Choose Another One',
        show_alert: false,
      })
      return
    } else if (p > 1) {
      if (picks.includes(h)) {
        bot.answerCallbackQuery(msg.id, {
          text: 'Already Picked By The Other Player Choose Another One',
          show_alert: false,
        })
        return
      }
    ...
    if (!newPlayers.players[p - 1].pick[0]) {
      ...
      newPlayers.players[p - 1].id = msg.from.id
      map.set(key, JSON.stringify(newPlayers))
      if (newPlayers.totalPlayers > 2) p++
      if (!msg.inline_message_id)
        data = {
          message_id: msg.message.message_id,
          chat_id: msg.message.chat.id,
      ...
      if (msg.chat_instance === INSTANCE) {
        ...
        if (newPlayers.totalPlayers < 3) {
          ...
          playerNames = ''
          players.forEach((p) => {
            if (!names.includes(`${p} , `)) playerNames += p + ' , '
          })
          return bot.editMessageText(
            game + '\n' + playerNames.slice(0, -2) + ' pick ',
            data
          )
        }
      }
        ...
        if (n.name)
          if (newPlayers.totalPlayers < 3)
            namesPick +=
              ...
              hands[n.pick[1] - 1] +
              '\n'
          else namesPick += n.name + ' is : ' + hands[n.pick[0] - 1] + '\n'
      })
      if (p > newPlayers.totalPlayers) {
        ...
        return bot.editMessageText(
          `Player ${p} / ${newPlayers.totalPlayers}  Pick `,
          data
        )
    } else {
  ...
  try {
    ...
    await newPlayers.players.forEach(async (n) => {
      ...
      if (n.name) {
        ...
        if (!n.turn) await names.push(n.name + ' , ')
        if (newPlayers.totalPlayers < 3)
          namesPick +=
            n.name +
            ' is : ' +
        ...
        if (n.score === ENDPOINT) return
      }
      picks.push(n.pick[0])
      if (newPlayers.totalPlayers < 3) picks.push(n.pick[1])
    })
    ...
    if (id.includes(msg.from.id)) {
      ...
      if (newPlayers.turned < newPlayers.totalPlayers) {
        ...
        if (!picks.includes(result))
          if (newPlayers.previousHands === handsReveal) return 0
          else {
      ...
      return bot.answerCallbackQuery(msg.id, {
        text: ' You Can Play In The Next Round , until then why not practice in the Bot ',
        show_alert: false,
      })
  } catch (error) {
...
const tournament = async (msg, state) => {
  try {
    ...
    let names = ''
    let text = ''
    if (tournamentPlayers.length > 5) text = 'Round ' + tournamentStatus.round
    else text = 'Final Round'
    await bot
      .sendMessage(USERNAME, text)
      .then((result) => bot.pinChatMessage(USERNAME, result.message_id))

    for (
      let i = 0, j = 0;
      i <= tournamentPlayers.length && tournamentPlayers.length >= 5;
      i++
    ) {
      if (i % 5 === 0 && i > 0) {
        let game = 'Game ' + (j + 1) + ' '
        if (tournamentPlayers.length <= 5) game = 'Final Game '
        map.set(j + 1, players)
        await bot
          .sendMessage(
            USERNAME,
            game + '\n' + names.slice(0, -2) + ' , start picking',
            data
          )
          .then((result) => {
            players[5] = result
    ...
    if (tournamentPlayers.length % 5 === 1) {
      ...
      bot.sendMessage(
        USERNAME,
        '➡️ ' +
          tournamentPlayers[tournamentPlayers.length - 1] +
          ' Has passed to Round ' +
